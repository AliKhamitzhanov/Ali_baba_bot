...
async def music_1(call: types.CallbackQuery):
    audio = open("music/Ирина Кайратовна, Shiza – Кок ту.mp3", 'rb')
    await bot.send_audio(call.message.chat.id, audio=audio)


async def music_2(call: types.CallbackQuery):
    audio = open("music/instasamka-moja-kiska-dlja-nego-vsegda-gotova_(kztrack.kz).mp3", 'rb')
    await bot.send_audio(call.message.chat.id, audio=audio)


async def music_3(call: types.CallbackQuery):
    audio = open("music/Lil_Nas_X_Jack_Harlow_-_INDUSTRY_BABY_73061759.mp3",'rb')
    await bot.send_audio(call.message.chat.id, audio=audio)


async def music_4(call: types.CallbackQuery):
    audio = open("music/Lizzo_-_About_Damn_Time_74115343.mp3", 'rb')
    await bot.send_audio(call.message.chat.id, audio=audio)


async def music_5(call: types.CallbackQuery):
    audio = open("music/Melanie_Martinez_-_Cake_48195644.mp3", 'rb')
    await bot.send_audio(call.message.chat.id, audio=audio)


async def music_6(call: types.CallbackQuery):
    audio = open("music/Naruto Shippuden OST — Opening №1 (www.lightaudio.ru).mp3", 'rb')
    await bot.send_audio(call.message.chat.id, audio=audio)


async def music_7(call: types.CallbackQuery):
    audio = open("music/opening_1_sezona_-_Klinok_rassekayushhijj_demonov_65423151.mp3", 'rb')
    await bot.send_audio(call.message.chat.id, audio=audio)


async def music_8(call: types.CallbackQuery):
    audio = open("music/OST_Naruto_shippuden_Ikimono-gakari_-_Blue_Bird_OP3_62967143.mp3", 'rb')
    await bot.send_audio(call.message.chat.id, audio=audio)


async def music_9(call: types.CallbackQuery):
    audio = open("music/Vance_Joy_-_Missing_Piece_73152511.mp3", 'rb')
    await bot.send_audio(call.message.chat.id, audio=audio)


async def music_10(call: types.CallbackQuery):
    audio = open("music/Vance_Joy_-_Riptide_47960565.mp3", 'rb')
    await bot.send_audio(call.message.chat.id, audio=audio)
